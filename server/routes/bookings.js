...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const bookings = await Booking.find({ user_id: req.user.id })
      .populate('product_id', 'name image_url type price')
      .sort({ created_at: -1 })
      .lean();
    res.json(bookings.map(b => ({
      ...b,
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Check time conflict for the same product on the same date
    const effectiveEnd = end_time || addBufferTime(preferred_time);
    const conflict = await Booking.findOne({
      ...
      $or: [
        { preferred_time: { $lt: effectiveEnd }, $expr: { $gt: [{ $ifNull: ['$end_time', addBufferTime('$preferred_time')] }, preferred_time] } }
      ]
    });

    // Simplified conflict check: any active booking on same product + date
    const simpleConflict = await Booking.findOne({
      product_id,
      preferred_date,
      status: { $in: ['pending', 'confirmed'] },
    ...
    const booking = await Booking.create({
      ...
      end_time: effectiveEnd,
      status: 'pending',
      reservation_expires_at: booking_type === 'reservation' ? getReservationExpiry(product.type === 'vehicle' ? 30 : 90) : null,
      delivery_method: delivery_method || 'pickup',
      notes: notes || null,
    });

    // Mark product as reserved if reservation
...
router.put('/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    booking.status = status;
    await booking.save();
    if (['cancelled'].includes(status) && booking.booking_type === 'reservation') {
      await Product.findByIdAndUpdate(booking.product_id, { status: 'available' });
    }
    res.json({ ...booking.toObject(), id: booking._id });
  } catch (err) {
