...
import { FiAlertTriangle, FiRefreshCw, FiList, FiPackage, FiSearch } from 'react-icons/fi';
            ...
            <tbody>
              {paginated.map(p => (
                <tr key={p.id} className={`border-t border-gray-100 ${p.stock_quantity <= 0 ? 'bg-red-50' : p.stock_quantity <= p.reorder_level ? 'bg-amber-50' : ''}`}>
                  ...
                  <td className="px-4 py-3"><span className={`font-bold ${p.stock_quantity <= 0 ? 'text-red-600' : p.stock_quantity <= p.reorder_level ? 'text-amber-600' : 'text-green-600'}`}>{p.stock_quantity}</span></td>
                  <td className="px-4 py-3">{p.reorder_level}</td>
                  <td className="px-4 py-3">{p.location || '-'}</td>
                  <td className="px-4 py-3">{p.times_sold}</td>
                  <td className="px-4 py-3">
                    {p.stock_quantity <= 0 ? <span className="badge badge-danger">Sold Out</span> : 
                     p.stock_quantity <= p.reorder_level ? <span className="badge badge-warning">Low Stock</span> :
                     <span className="badge badge-success">In Stock</span>}
                  </td>
                  <td className="px-4 py-3">
                    {p.type !== 'vehicle' && (
                      <button onClick={() => handleReorder(p)} className="btn-primary btn-sm flex items-center gap-1"><FiRefreshCw size={12} /> Reorder</button>
                    )}
                  </td>
                </tr>
              ))}
